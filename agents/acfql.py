...
class ACFQLAgent(flax.struct.PyTreeNode):
    ...
    def critic_loss(self, batch, grad_params, rng):
        ...
        ori_critic_loss = td_loss.mean()
        q_loss_clip = self.config.get("q_loss_clip", -1.0)
        if q_loss_clip > 0:
            q_clip_ratio = jnp.mean(td_loss > q_loss_clip)
            td_loss = jnp.clip(td_loss, max=q_loss_clip)
        else:
            q_clip_ratio = jnp.array(0.0)
        critic_loss = td_loss.mean()

        return critic_loss, {
            'critic_loss': critic_loss,
            'ori_critic_loss': ori_critic_loss,
    ...
    def actor_bc_flow_loss(self, batch, grad_params, rng):
        """Update the actor BC flow model."""
        if self.config["action_chunking"]:
            batch_actions = jnp.reshape(batch["actions"], (batch["actions"].shape[0], -1))  # fold in horizon_length together with action_dim
        else:
            batch_actions = batch["actions"][..., 0, :] # take the first one
    ...
    def _update_actor(self, batch):
        ...
        actor_loss, actor_info = self.actor_bc_flow_loss(batch, self.network.params['modules_actor_bc_flow'], actor_rng)
        return self.network.apply_gradients(actor_loss, **actor_info)
    ...
    def _update_critic(agent, batch):
        ...
        new_network, info = agent.network.apply_loss_fn(loss_fn=loss_fn)
        agent.target_update(new_network, 'critic')
        return agent.replace(network=new_network, rng=new_rng), {f'critic/{k}': v for k, v in info.items()}
...
def get_config():
    ...
    config = ml_collections.ConfigDict(
        dict(
            ...
            weight_decay=0.,
            q_loss_clip=-1.0,  # Clip value for critic TD loss elements (-1.0 = no clipping).
        )
    )
    return config
