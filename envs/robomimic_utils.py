...
import sys
robomimic_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "robomimic"))
...
def _check_dataset_exists(env_name):
    ...
    download_folder = os.path.join(
        os.environ.get("ROBOMIMIC_DATA_DIR", expanduser("~/.robomimic")),
        task,
        dataset_type,
        file_name
    )
...
def _check_subset_dataset_exists(env_name, subset_path=None):
    """
    Check if a subset dataset exists.
    
    Args:
        env_name: Environment name (e.g., "square-mh-low_dim")
        subset_path: Optional path to subset dataset. If None, uses default path.
    
    Returns:
        Path to the subset dataset
    """
    if subset_path is None:
        # Use default subset path in workspace
        task, dataset_type, hdf5_type = env_name.split("-")
        subset_path = os.path.join(
            os.path.dirname(__file__),
            "..",
            "data",
            f"{task}_{dataset_type}_{hdf5_type}_v15_subset30.hdf5"
        )
    
    subset_path = os.path.abspath(subset_path)
    assert os.path.exists(subset_path), f"Subset dataset not found at {subset_path}"
...
def get_subset_dataset(env, env_name, subset_path=None):
    """
    Load a subset dataset (e.g., with only first 30 trajectories).
    ...
    Returns:
        Dataset object containing the subset data
    """
    dataset_path = _check_subset_dataset_exists(env_name, subset_path)
    
    print(f"Loading subset dataset from: {dataset_path}")
    ...
    print(f"Subset dataset: {num_demos} trajectories, {num_timesteps} timesteps")
